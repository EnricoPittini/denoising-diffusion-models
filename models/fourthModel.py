...
def Upsample(dim, device='cpu'):
    return nn.ConvTranspose2d(dim, dim, 4, 2, 1, device=device)

def Downsample(dim, device='cpu'):
    return nn.Conv2d(dim, dim, 4, 2, 1, device=device)
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups = 8, device='cpu'):
        super().__init__()
        self.proj = nn.Conv2d(dim, dim_out, 3, padding = 1, device=device)
        self.norm = nn.GroupNorm(groups, dim_out, device=device)
        self.act = nn.SiLU()
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, dim_out, *, time_emb_dim=None, groups=8, device='cpu'):
        super().__init__()
        self.mlp = (
        ...
        self.block1 = Block(dim, dim_out, groups=groups, device=device)
...
class ConvNextBlock(nn.Module):
    ...
    def __init__(self, dim, dim_out, *, time_emb_dim=None, mult=2, norm=True, device='cpu'):
        super().__init__()
        self.mlp = (
            nn.Sequential(nn.GELU(), nn.Linear(time_emb_dim, dim, device=device))
            if exists(time_emb_dim)
            else None
        )

        self.ds_conv = nn.Conv2d(dim, dim, 7, padding=3, groups=dim, device=device)
...
class FourthModel(nn.Module):
    def __init__(
        ...
        self.init_conv = nn.Conv2d(channels, init_dim, 7, padding=3, device=device)
