...
class TimestepPredictionDataset(torch.utils.data.Dataset):
    """Custom Dataset for generating noisy images for
    the forward diffusion process,
    given a dataset and a variance schedule.
    ...
    *args, **kwargs
    ...
    def __init__(self, diffusion_dataset, *args, **kwargs):
        self.diffusion_dataset = diffusion_dataset       

    def __getitem__(self, idx):
        (x_noisy, t), noise = self.diffusion_dataset[idx]

        return x_noisy, t

    def __len__(self):
        return len(self.diffusion_dataset)
